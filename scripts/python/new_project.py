...
CMD = 'newproject'  # the name of the command alias in your shell - used to start this script
...
def init_project(structure='basic'):
    ...
    subprocess.call(f'git config --local core.sshCommand "ssh -i ~/.ssh/id_rsa_{REPO}"', shell=True)
...
user = subprocess.check_output(["git", "config", "user.name"]).decode("utf-8").replace('\n', '')
email = subprocess.check_output(["git", "config", "user.email"]).decode("utf-8").replace('\n', '')
DEFAULTS['GIT_USER'] = user
DEFAULTS['GIT_EMAIL'] = email

# handling the arguments
parser = ArgParser(
    description=__doc__, prog=f'{CMD}',
...
parser.add_argument('name', metavar='projectname',
                    help="The name of the project you want to create, doesn't accept spaces or underscores",
                    nargs='*')
parser.add_argument('--app', metavar=', '.join(structures),
                    default='basic',
                    const='basic',
                    nargs='?',
                    choices=structures,
                    help=f'create prebuild application structure')
parser.add_argument('--repo', metavar=', '.join(repository_providers),
                    nargs='?',
...
if __name__ == '__main__':
    ...
    if not args.name:
        while not NEW or '_' in NEW or ' ' in NEW:
            NEW = input('please enter a valid projectname (no spaces, no underscores) > ')
    ...
    if args.repo:
        ...
        if REPO not in [r.upper() for r in repository_providers]:
            parser.error(f'the repository provided in the settings ({settings["REPO"]}) is not supported')
