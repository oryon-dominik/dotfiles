...
$API = $Env:OPEN_WEATHERMAP_KEY

$error_free = $true

$JSONurl = "api.openweathermap.org/data/2.5/weather?q=$City,$Country&units=metric&appid=$API&type=accurate&mode=json"
$XMLurl = "api.openweathermap.org/data/2.5/weather?q=$City,$Country&units=metric&appid=$API&type=accurate&mode=xml"


try { 

	$JSONresults = Invoke-WebRequest $JSONurl
	[xml]$XMLresults = Invoke-WebRequest $XMLurl
	...
	$varsunset = [TimeZone]::CurrentTimeZone.ToLocalTime(([datetime]'1/1/1970').AddSeconds($JSONsunset))
