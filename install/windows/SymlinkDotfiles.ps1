...
function SymlinkDotfiles {
    ...
    if ($starship -eq $true) {
        # starship
        Remove-Item -path "$env:USERPROFILE/.config/starship.toml" -recurse -force -ErrorAction SilentlyContinue
    ...
    if ($vim -eq $true) {
        ...
        Remove-Item -path "$env:USERPROFILE/.config/vim" -recurse -force -ErrorAction SilentlyContinue
    ...
    if ($micro -eq $true) {
        # micro
        Remove-Item -path "$env:USERPROFILE/.config/micro" -recurse -force -ErrorAction SilentlyContinue
        New-Item -Path "$env:USERPROFILE/.config" -Name "micro" -ItemType "directory" -Force
